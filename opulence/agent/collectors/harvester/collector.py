...
class TheHarester(DockerCollector):
    ...
    def scan(self, target):
        ...
        for item, _ in self.findall_regex(data, r"\[\*\] IPs found: \d+\n-------------------\n((.|\n)*)\n\[\*\] Emails found"):
            for ip in item.split("\n"):
                if ip:
                    yield IPv4(address=ip)

        for item, _ in self.findall_regex(data, r"\[\*\] Emails found: \d+\n----------------------\n((.|\n)*)\n\[\*\] Hosts found"):
            for email in item.split("\n"):
                if email:
                    yield Email(address=email)

        for item, _ in self.findall_regex(data, r"\[\*\] Hosts found: \d+\n---------------------\n((.|\n)*)"):
